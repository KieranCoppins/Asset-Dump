...
3. **Redundant lookup-array lines are dropped.** An array-element line (`SomeProperty(N)=value`) is dropped when every identifier embedded in its value — a 32-hex GUID, or an object reference's bare name (the `Name`/`ID` half of a `ClassPath'Name'`-shaped literal) — is genuinely declared elsewhere in the same top-level object's own export, as either an `ID=`/`Id=` field (a standalone line, or one sub-field of a single-line struct literal, e.g. how StateTree tasks/states/transitions are exported) or a `Begin Object ... Name="..."` header. This is one general mechanism covering what were previously two separate special cases: StateTree's `IDToStateMappings`/`IDToNodeMappings`/`IDToTransitionMappings` (GUID → array-index lookup tables whose GUIDs already appear as `ID=`/`Id=` on the corresponding state/task/transition elsewhere in the same dump) and an `EdGraph`'s `Nodes(N)="..."` index array (every node it names is already fully declared, with all its properties, via its own `Begin Object` block elsewhere in the same graph). Two safeguards keep this from misfiring on the real behavioral entries that carry the same shape: an identifier only counts as "declared" when it's found nested inside some child entity's own scope, not as a direct property of the top-level object itself (which is exactly where compiled, object-wide lookup/cross-reference tables live); and among competing same-shaped entries for the same identifier, only a *strictly longer* one counts as the declaration, so a real, data-rich entry (e.g. a StateTree task, which restates its own guid at the end of its own line) is never treated as redundant merely because a thinner lookup elsewhere also mentions it. An array whose values aren't identifiers at all (e.g. `UMovieSceneNodeGroup::Nodes`, plain dotted path strings with no object-reference quoting) or that reference something not declared elsewhere is never touched.
4. **Known-default Blueprint pin sub-fields are stripped.** Graph pins (`CustomProperties Pin (...)`) are exported through a code path (`UEdGraphPin::ExportTextItem`) that, unlike normal property export, never compares against a default for several fields — they're restated whether or not they hold a non-default value. Rather than a hand-maintained value table, the commandlet computes each field's real default at runtime by rendering a genuine default-constructed engine instance through the exact same `FProperty` export call the real exporter uses, and strips a field only when it is present with exactly that computed value; a field whose value differs from its default (e.g. `bHidden=True`, an actual `LinkedTo=`) is left completely untouched. This tracks every current and future default-emitting pin field automatically — including every `FEdGraphPinType` sub-field — rather than a fixed, hand-typed list that can silently drift from the real engine default across a UE version upgrade.
5. **`PersistentGuid` is dropped unconditionally.** It's bookkeeping Unreal uses to reconnect a pin's wires across a Blueprint recompile — not part of what the graph does — so it's removed regardless of value.
6. **`PinFriendlyName` is dropped unconditionally.** It's the Blueprint editor's cosmetic display label for a pin (e.g. a pin literally named `self` is shown to a designer as "Target"). It is always a reworded form of `PinName` and never adds structural or behavioral information.
7. **Remaining GUIDs are aliased to short IDs, scoped to a single commandlet run.** Every remaining `FGuid`-shaped hex value (node IDs, pin IDs, struct IDs, etc.) is replaced with a short alias: the nearby human-readable `Name="..."` value when one can be paired with it (disambiguated on collision — two different objects with the same display name get suffixed `_2`, `_3`, ...), otherwise a sequential `G<N>`. The same real GUID always maps to the same alias everywhere it's referenced within that run, so cross-references (Blueprint pin wiring, State Tree property bindings, etc.) stay fully traceable. Aliases are **not** stable across separate invocations of the commandlet.
